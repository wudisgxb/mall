...
module.exports = {
    ...
    async getUserShoppingCart (ctx, next) {
        ctx.checkParams('tenantId').notEmpty();
        ctx.checkParams('tableName').notEmpty();
    ...
    async updateUserShoppingCartAddById (ctx, next) {
    ...
    async updateUserShoppingCartEditById (ctx, next) {
        ctx.checkParams('id').notEmpty().isInt().toInt();
